...
def compute_grpo_outcome_advantage(
    ...
    ER_high_prob_block_coeff = ER_config.get("ER_high_prob_block_coeff", 0.0)
    ER_high_prob_block_threshold = ER_config.get("ER_high_prob_block_threshold", 0.9)
    ER_high_prob_block_window_size = ER_config.get("ER_high_prob_block_window_size", 3)
    ER_only_pos = bool(ER_config.get("ER_only_pos", True))

    scores = token_level_rewards.sum(dim=-1)
    metrics = {}

    with torch.no_grad():
        is_postive = (scores > 0) # (bs, )
        id2lengths = defaultdict(list)
        id2correct = defaultdict(list)

        lengths = response_mask.sum(dim=-1)
        total_lengths = lengths.to(torch.float32)
        pos_lengths = lengths[is_postive].to(torch.float32)
        neg_lengths = lengths[~is_postive].to(torch.float32)
        
        bsz = scores.shape[0]
        for i in range(bsz):
            id2lengths[index[i]].append(lengths[i])
            id2correct[index[i]].append(is_postive[i])
        ...
        if data is not None:
            ...
            # 滑动窗口,长度window_size计算概率乘积
            prod_prob = torch.zeros_like(old_log_probs) # (bs, response_length)
            for i in range(response_length - ER_high_prob_block_window_size + 1):
                if i > 0:
                    prod_prob[:, i] = torch.exp(old_log_probs[:, i:i+ER_high_prob_block_window_size].sum(-1))

            lw_entropy_count = ((prod_prob > ER_high_prob_block_threshold) * response_mask).sum(-1)
            
            if ER_only_pos:
                scores = scores - ER_high_prob_block_coeff * lw_entropy_count * is_postive
            else:
                scores = scores - ER_high_prob_block_coeff * lw_entropy_count
    ...
    metrics.update({
        "GRPO/max_response_num": max(total_lengths),
        "GRPO/max_pos_response_num": max(pos_lengths),
        "GRPO/max_neg_response_num": max(neg_lengths),

        "GRPO/min_response_num": min(total_lengths),
        "GRPO/min_pos_response_num": min(pos_lengths),
        "GRPO/min_neg_response_num": min(neg_lengths),

        "GRPO/mean_response_num": total_lengths.mean().cpu().item(),
        "GRPO/mean_pos_response_num": pos_lengths.mean().cpu().item(),
        "GRPO/mean_neg_response_num": neg_lengths.mean().cpu().item(),
    })
